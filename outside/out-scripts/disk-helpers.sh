...
scan_devices_hd() {
  ...
  while true; do
    ...
    ((${#devices[@]})) && break
    warning "No hard drives found. Insert media and press Enter…"
    read -r
  done

  select_device "Select target hard drive" DEVICE "${devices[@]}"
  printf '%s\n' "$TARGET_DISK"
}
