...
def enable_user_serv(
    installation: Installer, units: list[UserService], username: str
) -> None:
    for unit in units:
...
def multi_user_funcs(installation: Installer, user: User, nc: NoahConfig):
    ...
    if serv_conf := nc.user_services_config:
        enable_user_serv(installation, serv_conf.services, user.username)
